...
export class SignupComponent{
  ...
  ngOnInit() {
    this.miniLoading();
    this.dataService.fetchData('signup', 'auth', () => {
    this.signup = this.dataService.getData();
    setTimeout(() => {
  ...
  regexForPassword = /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$/

  toggle = true

  error: string = `<div class="alert alert-danger container  p-1 my-3 col-12 col-sm-8  col-xl-5  text-center text-capitalize" id="error" role="alert" >Email Already Exists</div>`
